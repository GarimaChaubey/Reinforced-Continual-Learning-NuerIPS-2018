    ...
    def evaluate_action(self,var_list,actions,task_id):
        with tf.Graph().as_default() as g:
            ...
            with tf.name_scope("model"):
                # Get input size and num_classes from the first task's data shape
                input_size = self.task_list[0][0].shape[1]
                num_classes = self.task_list[0][1].shape[1]
                self.x = tf.compat.v1.placeholder(tf.float32, shape=[None, input_size]) 
                self.y = tf.compat.v1.placeholder(tf.float32, shape=[None, num_classes])
                fc1 = tf.Variable(tf.concat([var_list[0],tf.random.truncated_normal((var_list[0].shape[0],actions[0]),stddev=0.01)],axis=1))
                ...
                value = tf.concat([var_list[2],tf.random.truncated_normal((actions[0],old_shape[1]),stddev=0.01)],axis=0)
                fc2 = tf.Variable(tf.concat([value,tf.random.truncated_normal((actions[0]+old_shape[0],actions[1]),stddev=0.01)],axis=1))
                ...
                fc3 = tf.Variable(tf.random.truncated_normal((var_list[4].shape[0]+actions[1],var_list[4].shape[1])))
                mask_fc3 = np.ones_like(fc3)
                b3 = tf.Variable(tf.constant(0.1,shape=(var_list[4].shape[1],)))
                mask_b3 = np.ones_like(b3)
            total_theta = tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.TRAINABLE_VARIABLES, scope='model')
            ...
            x_normalized = tf.nn.l2_normalize(self.x, axis=1)
            x_dropped = tf.nn.dropout(x_normalized, rate=0.2)
            
            # First layer with dropout
            h_fc1 = tf.nn.relu(tf.matmul(x_dropped, fc1) + b1)
            h_fc1 = tf.nn.dropout(h_fc1, rate=0.3)
            
            # Second layer with dropout
            h_fc2 = tf.nn.relu(tf.matmul(h_fc1, fc2) + b2)
            h_fc2 = tf.nn.dropout(h_fc2, rate=0.3)
            
            # Output layer
            h_fc3 = tf.matmul(h_fc2, fc3) + b3

            # Adjust regularization based on input size (CIFAR vs MNIST)
            reg_factor = 0.00001 if self.task_list[0][0].shape[1] == 3072 else 0.0001
            loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(labels = self.y,logits = h_fc3)) + \
                   reg_factor*(tf.nn.l2_loss(fc1) + tf.nn.l2_loss(fc2) + tf.nn.l2_loss(fc3))
            ...
            for epoch in range(self.epochs):
                ...
                if epoch%4==0 or epoch==self.epochs-1:
                    print("task:%d,test accuracy:%f"%(task_id,accuracy_test))
